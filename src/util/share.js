export let AutoScroll = function () {
    AutoScroll.prototype.txtScroll = function (ele,direction) {
        ele.style.position = "absolute";
        let total = ele.querySelectorAll("li").length;
        let count = 0;
        setInterval(function(){
            count++;
            //message.success(`${total} ==== ${count} ===== ${ele.offsetTop}`);
            setDirection(ele,direction);
            if(count === total) {
                count = 0;
                setDirection(ele,direction,true);
            }
        },3000);
    };
    function setDirection(ele,direction,bool=false){
        if(bool){
            ele.style.top = ele.querySelectorAll("li")[0].offsetHeight + "px";
        }
        switch(direction){
            case "top":
                ele.style.top = "-" + (ele.querySelectorAll("li")[0].offsetHeight-ele.offsetTop) + "px";
                break;
            // case "bottom":
            //     ele.style.top = -value;
            //     break;
            case "left":
                ele.style.left = ele.width;
                break;
            // case "right":
            //     ele.style.right = -value;
            //     break;
            default:
                break;
        }
...
export function TextAutoScroll() {
    var _this = this;
    this.ScrollTypeEnum = {
        LeftRight: 0,//左右滚动
        Left: 1,//像左滚动
